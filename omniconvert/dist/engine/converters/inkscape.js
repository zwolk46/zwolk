...
export const inkscapeConverter = {
    id: "inkscape",
    label: "Inkscape",
    ...
    availability: async () => {
        ...
        if (!inkscape)
            return { ok: false, reason: "Executable `inkscape` not found in PATH" };
        return { ok: true, executables: { inkscape } };
    },
    supports: (from, to) => {
        if (from === to)
            return false;
        if (from === "svg" && ["png", "pdf"].includes(to))
            return true;
        if (from === "pdf" && to === "svg")
            return true;
        return false;
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const inkscape = await findExecutable(["inkscape"]);
        if (!inkscape)
            throw new Error("Inkscape not available (`inkscape` not found)");
        ...
        if (code !== 0)
            throw new Error(`Inkscape failed with exit code ${code}`);
