...
export const libreofficeConverter = {
    id: "libreoffice",
    label: "LibreOffice",
    ...
    availability: async () => {
        ...
        if (!soffice)
            return { ok: false, reason: "Executable `soffice` not found in PATH" };
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const soffice = await findExecutable(["soffice", "libreoffice"]);
        if (!soffice)
            throw new Error("LibreOffice not available (`soffice` not found)");
        ...
        if (code !== 0)
            throw new Error(`LibreOffice failed with exit code ${code}`);
...
async function locateConvertedFile(outputDir, to, expected) {
    try {
        ...
        if (!match)
            throw new Error("LibreOffice finished but output file was not found");
