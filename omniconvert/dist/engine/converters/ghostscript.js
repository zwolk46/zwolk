...
export const ghostscriptConverter = {
    id: "ghostscript",
    label: "Ghostscript",
    ...
    availability: async () => {
        ...
        if (!gs)
            return { ok: false, reason: "Executable `gs` not found in PATH" };
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const gs = await findExecutable(["gs"]);
        if (!gs)
            throw new Error("Ghostscript not available (`gs` not found)");
        ...
        if (code !== 0)
            throw new Error(`Ghostscript failed with exit code ${code}`);
