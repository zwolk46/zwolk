...
export const sevenZipConverter = {
    id: "7zip",
    label: "7-Zip",
    ...
    availability: async () => {
        ...
        if (!exe)
            return { ok: false, reason: "Executable `7z`/`7za`/`7zz` not found in PATH" };
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const exe = await findExecutable(["7z", "7za", "7zz"]);
        if (!exe)
            throw new Error("7-Zip not available (`7z` not found)");
        ...
        try {
            ...
            if (extracted.code !== 0)
                throw new Error(`7-Zip extract failed with exit code ${extracted.code}`);
            // Re-pack
            ...
            if (added.code !== 0)
                throw new Error(`7-Zip pack failed with exit code ${added.code}`);
