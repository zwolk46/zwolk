...
export const ffmpegConverter = {
    ...
    availability: async () => {
        ...
        if (!ffmpeg)
            return { ok: false, reason: "Executable `ffmpeg` not found in PATH" };
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const ffmpeg = await findExecutable(["ffmpeg"]);
        if (!ffmpeg)
            throw new Error("FFmpeg not available (`ffmpeg` not found)");
