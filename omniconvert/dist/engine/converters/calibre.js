...
export const calibreConverter = {
    ...
    availability: async () => {
        ...
        if (!ebookConvert)
            return { ok: false, reason: "Executable `ebook-convert` not found in PATH" };
    ...
    convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
        const ebookConvert = await findExecutable(["ebook-convert"]);
        if (!ebookConvert)
            throw new Error("Calibre not available (`ebook-convert` not found)");
        ...
        if (code !== 0)
            throw new Error(`Calibre failed with exit code ${code}`);
