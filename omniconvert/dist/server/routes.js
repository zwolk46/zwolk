...
export async function registerRoutes(app) {
    ...
    app.post("/api/jobs", async (req, reply) => {
            ...
            for await (const part of parts) {
                if (part.type === "file") {
                    ...
                    if (part.fieldname === "to")
                        body.to = String(part.value || "");
                    if (part.fieldname === "converter")
                        body.converter = String(part.value || "");
                }
            }
        }
        catch (err) {
            if (uploadDir)
                await safeRm(uploadDir);
            throw err;
        }
        ...
        queue.enqueue(job.id, async (signal, log) => {
    ...
    app.post("/api/jobs/:id/cancel", async (req, reply) => {
        ...
        if (!canceled)
            return { ok: false, error: "Not cancelable (already finished or not started)" };
