...
export const imagemagickConverter: Converter = {
  id: "imagemagick",
  label: "ImageMagick",
  description: "Image conversion (raster/vector) via `magick`.",
  availability: async () => {
    const magick = await findExecutable(["magick"]);
    if (!magick) return { ok: false, reason: "Executable `magick` not found in PATH" };
  ...
  convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
    const magick = await findExecutable(["magick"]);
    if (!magick) throw new Error("ImageMagick not available (`magick` not found)");
    ...
    if (code !== 0) throw new Error(`ImageMagick failed with exit code ${code}`);
