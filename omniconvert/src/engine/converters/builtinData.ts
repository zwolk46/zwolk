...
export const builtinDataConverter: Converter = {
  id: "builtin-data",
  label: "Built-in (Data)",
  ...
  supports: (from, to) => {
    ...
    if (from === "json" && to === "yaml") return true;
    if (from === "yaml" && to === "json") return true;
    if (from === "csv" && to === "xlsx") return true;
    if (from === "xlsx" && to === "csv") return true;
    return false;
  },
  convert: async ({ inputPath, originalFilename, from, to, outputDir }) => {
    ...
    if (from === "xlsx" && to === "csv") {
      ...
      if (!firstName) throw new Error("XLSX has no sheets");
