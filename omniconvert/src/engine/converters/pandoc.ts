...
export const pandocConverter: Converter = {
  ...
  availability: async () => {
    ...
    if (!pandoc) return { ok: false, reason: "Executable `pandoc` not found in PATH" };
  ...
  convert: async ({ inputPath, originalFilename, to, outputDir, ctx }) => {
    const pandoc = await findExecutable(["pandoc"]);
    if (!pandoc) throw new Error("Pandoc not available (`pandoc` not found)");
    ...
    if (code !== 0) throw new Error(`Pandoc failed with exit code ${code}`);
