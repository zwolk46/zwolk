...
export type Format = {
  ...
  label: string;
  extensions: string[];
  mediaType: "image" | "video" | "audio" | "document" | "archive" | "data" | "font" | "other";
};
...
export function inferFormatFromFilename(filename: string): FormatId | null {
  const base = filename.toLowerCase();
  if (base.endsWith(".tar.gz")) return "tgz";
...
export async function detectFormatFromFile(
  ...
  if (fallbackFilename) return inferFormatFromFilename(fallbackFilename);
  return null;
}
