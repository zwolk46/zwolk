...
import type { ConversionResult, ConvertContext, Converter } from "./types.js";
...
export async function runConversion(args: {
  ...
  const from =
    (await detectFormatFromFile(inputPath, originalFilename)) ||
  ...
  const ctx: ConvertContext = { signal: args.signal, log: args.log };
...
async function chooseConverter(args: {
  ...
  if (args.requestedConverter) {
    ...
    if (!forced) throw new Error(`Requested converter not valid for ${from} -> ${to}`);
    const avail = await forced.availability();
    if (!avail.ok) throw new Error(`Requested converter unavailable: ${avail.reason || "unknown"}`);
    return forced;
  }

  for (const c of candidates) {
    const avail = await c.availability();
    if (avail.ok) return c;
