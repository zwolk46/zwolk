...
**Where the app's context lives.** `wc/CLAUDE.md` is the source of truth for the World Cup app — data shapes, the live API + polling/budget rules, the static vs. live split, and how the data files join. `wc/data/enrichment/ENRICHMENT_HANDOFF.md` documents the enrichment sources. `wc/docs/api-reference/` has per-source API notes. Read these before touching the WC app rather than inferring from code.
